...
function adicionarProduto(id){
  ...
  else carrinho.push({id:p.id,nome:p.nome,preco:p.preco,quantidade:1});
...
function aumentarQuantidade(id){
  ...
  if(i){ i.quantidade++; mostrarCarrinho(); }
}
...
function calcularTotal(){
  return carrinho.reduce((s,x) => s + x.preco * x.quantidade,0);
}
  ...
  if(!carrinho.length){
    area.innerHTML = `
      <div class="cart-empty">
        ...
        <span>Toque no + de um produto para adicionar.</span>
      </div>`;
    atualizarTotal();
    return;
  }
...
/* Os itens ficam dentro de observacoes para não exigir outra tabela. */
function codificarItens(itens,obs){
  return String(obs || "") + "\n\n[ML_ITENS]" +
    encodeURIComponent(JSON.stringify(itens)) +
    "[/ML_ITENS]";
}
...
function nomeCliente(p){
  return p?.Cliente ?? p?.cliente ?? p?.nome ?? "Sem nome";
}

function totalPedido(p){
  return Number(p?.total ?? p?.Total ?? 0);
}

function dataPedido(p){
  return p?.created_at || p?.data_hora || p?.dataHora || p?.createdAt || "";
}

function numeroPedido(p,i){
  const v = p?.id ?? p?.numero ?? p?.Numero;
  return v !== undefined && v !== null && v !== ""
    ? String(v).slice(-6).padStart(3,"0")
...
function mostrarComandas(){
  ...
  const abertos = pedidos.filter(p => {
    const status = p.status_pedido || extrairStatus(p.observacoes);
  ...
  area.innerHTML = abertos.map(p => {
    ...
    const resumo = itens.length
      ? itens.map(x => `${x.quantidade}x ${escapar(x.nome)}`).join(", ")
      : "Pedido registrado";
    ...
    if(status === "preparo"){
      botaoStatus = `
        <button type="button" class="status-action"
          onclick="event.stopPropagation(); alterarStatusPedido('${String(p.id)}','entrega')">
          🛵 Saiu para entrega
        </button>`;
    }else if(status === "entrega"){
      botaoStatus = `
        <button type="button" class="status-action delivered"
          onclick="event.stopPropagation(); alterarStatusPedido('${String(p.id)}','entregue')">
          ✅ Entregue
        </button>`;
    }
    ...
    return `
      <div class="order-card" style="padding:14px;border-bottom:1px solid #eee">
        <div onclick="selecionarPedido('${String(p.id)}')" style="cursor:pointer">
          <strong>#${numeroPedido(p,0)} - ${escapar(nomeCliente(p))}</strong>
          <div>${resumo}</div>
          <strong>${moeda(totalPedido(p))}</strong>
          <div class="order-status">${statusLabel(status)}</div>
        </div>

        <div class="status-action-row">${botaoStatus}</div>

        <div class="status-action-row">
          <button type="button" class="cancel-order-btn"
            onclick="event.stopPropagation(); if(confirm('Tem certeza que deseja cancelar este pedido?')) alterarStatusPedido('${String(p.id)}','cancelado')">
            ❌ Cancelar pedido
          </button>
        </div>
      </div>`;
...
function atualizarResumoHistorico(){
  ...
  if(!box){
    ...
    box.style.cssText =
      "display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:0 0 16px";
  ...
  box.innerHTML = `
    <div class="summary-card">
      <strong>💰 Vendas de hoje</strong>
      <div style="font-size:24px;font-weight:800">${moeda(total)}</div>
    </div>
    <div class="summary-card">
    ...
    <div class="summary-card">
      ...
      <div style="font-size:24px;font-weight:800">${moeda(media)}</div>
    </div>`;
}

function selecionarPedido(id){
  pedidoSelecionado = pedidos.find(p => String(p.id) === String(id)) || null;
  abrirPagina("impressao");
  mostrarImpressao();
}

function mostrarImpressao(){
  const area = pegar("printPreview");
  if(!area) return;

  if(!pedidoSelecionado){
    area.innerHTML = `<div class="receipt-empty">Selecione um pedido para visualizar.</div>`;
    return;
  }
  ...
  const linhas = itens.length
    ? itens.map(x => `
      <div style="display:flex;justify-content:space-between;gap:10px;margin:5px 0">
        <span>${x.quantidade}x ${escapar(x.nome)}</span>
        <strong>${moeda(x.preco * x.quantidade)}</strong>
      </div>`).join("")
    : `<div>Nenhum item detalhado salvo.</div>`;

  area.innerHTML = `
    <div class="receipt-content">
      <div style="text-align:center;font-weight:bold;font-size:18px">MIGUEL LANCHES</div>
      <hr>
      <div><strong>PEDIDO:</strong> #${numeroPedido(p,idx < 0 ? 0 : idx)}</div>
      <div><strong>DATA/HORA:</strong> ${formatarData(dataPedido(p))}</div>
      <hr>
      ...
      <div style="display:flex;justify-content:space-between;font-size:17px">
        <strong>TOTAL:</strong><strong>${moeda(totalPedido(p))}</strong>
      </div>
      <hr>
...
async function alterarStatusPedido(id,status){
  ...
  if(status === "entrega"){
    ...
    if(numero){
      ...
      const url = "whatsapp://send?phone=" + numero + "&text=" + mensagem;
...
async function finalizarPedido(){
  ...
  try{
    ...
    const pedidoLocal = data || {
      ...dados,
      id: Date.now(),
      created_at: new Date().toISOString(),
      status_pedido:"preparo",
      __entregue:false
    };
...
function abrirPagina(nome){
  ...
  const pagina = pegar("page-" + nome);
  if(pagina) pagina.classList.add("active-page");

  document.querySelectorAll(".menu-item").forEach(b =>
    b.classList.toggle("active",b.dataset.page === nome)
  );
  ...
  if(nome === "comandas" || nome === "historico" || nome === "impressao"){
    carregarPedidos();
  }

  if(nome === "impressao") mostrarImpressao();

  const sidebar = pegar("sidebar");
...
<html>
<head>
<title>Comanda Miguel Lanches</title>
<style>
body{font-family:Arial,sans-serif;width:80mm;margin:0 auto;padding:8px;font-size:12px}
