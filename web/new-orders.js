...
(function(){
  ...
  window.imprimirPedidoNovo=function(id){
    const p=getOrders().find(x=>String(x.id)===String(id));
    ...
    const rows=items.map(x=>{
      ...
      return `<div class="item">
        <div class="row"><span><b>${esc(x.quantidade)}x</b> ${esc(x.nome)}</span>
        <b>${money(Number(x.preco||0)*Number(x.quantidade||1))}</b></div>
    ...
    w.document.write(`<!doctype html><html><head><meta charset="utf-8">
      ...
      <div class="center"><div class="logo">MIGUEL LANCHES</div>
      <div class="line"></div><b>PEDIDO #${esc(String(p.id).slice(-5))}</b><br>
      ${esc(new Date(p.created_at).toLocaleString("pt-BR"))}</div>
      <div class="line"></div>
  ...
  window.prepararPedidoNovo=async function(id){
    ...
    const r=await window.db.from("pedidos").update({observacoes:obs}).eq("id",p.id);
    ...
    if(p.telefone){
      ...
      const msg=encodeURIComponent(
        `Olá, ${p.Cliente||"cliente"}! 😊 Recebemos seu pedido e já vamos começar a preparar. Obrigado pela preferência! 🍔`
      );
  ...
  window.renderPedidosNovos=function(){
    ...
    box.innerHTML=novos.length ? `
      <div class="panel new-orders-panel">
        <div class="new-orders-title">
          <div><h2>🔔 Pedidos novos</h2><small>Escolha uma ação para cada pedido.</small></div>
          <strong>${novos.length}</strong>
        </div>
        ${novos.map(p=>{
          const items=getItems(p.observacoes);
          return `<div class="new-order-card">
            ...
            <div class="new-order-items">
              ${items.length ? items.map(x=>`${esc(x.quantidade)}x ${esc(x.nome)}`).join("<br>") : "Pedido"}
            </div>
            <div class="new-order-actions">
              <button class="action print-new" onclick="imprimirPedidoNovo('${p.id}')">🖨️ Imprimir</button>
              <button class="action green" onclick="prepararPedidoNovo('${p.id}')">🍔 Preparar pedido</button>
              <button class="action red" onclick="recusarPedidoNovo('${p.id}')">✕ Recusar</button>
            </div>
          </div>`;
        }).join("")}
      </div>` : "";
  ...
  /* Executa depois que o app principal termina de carregar/renderizar. */
  const oldRenderAll=window.renderAll;
  window.renderAll=function(){
    if(typeof oldRenderAll==="function")oldRenderAll();
    setTimeout(window.renderPedidosNovos,50);
  };

  setTimeout(window.renderPedidosNovos,500);
  setInterval(window.renderPedidosNovos,2000);
})();